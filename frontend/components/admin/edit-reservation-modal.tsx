...
interface EditReservationModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (id: string, data: any) => Promise<{ success: boolean; message?: string; error?: string }>
  reservation: Reservation | null
  tables: Table[]
}

export function EditReservationModal({ isOpen, onClose, onSave, reservation, tables }: EditReservationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!reservation || !formData.customerName || !formData.customerPhone || !formData.date || !formData.time || !formData.tableId) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-w-[95vw] max-h-[90vh] overflow-y-auto mx-4">
        <DialogHeader>
          <DialogTitle className="text-center text-lg sm:text-xl">Edit Reservation</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
          ...
          <Card className="text-xs sm:text-sm">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm sm:text-base">Current Reservation Info</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">{new Date(reservation.createdAt).toLocaleDateString()}</p>
                </div>
                <div>
                  <p className="font-medium">Last Updated</p>
                  <p className="text-gray-600">{new Date(reservation.updatedAt || reservation.createdAt).toLocaleDateString()}</p>
                </div>
              </div>
            </CardContent>
          </Card>
