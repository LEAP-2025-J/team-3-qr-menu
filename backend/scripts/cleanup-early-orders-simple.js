...
async function deleteEarlyOrders(orders, confirm = false) {
  if (!confirm) {
    console.log("\n⚠️  Захиалгуудыг устгахын тулд --confirm параметр өгнө үү.");
    return;
  }
  ...
  for (const order of orders) {
    try {
      ...
      if (order.table) {
        ...
        // Хэрэв энэ захиалга currentOrder байсан бол null болгох
...
async function main() {
  try {
    ...
    if (earlyOrders.length === 0) {
      ...
      console.log("   node cleanup-early-orders-simple.js --confirm");
