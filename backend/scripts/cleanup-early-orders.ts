 ...
 */
async function deleteEarlyOrders(orders: any[], confirm: boolean = false) {
  if (!confirm) {
    console.log(
  ...
  for (const order of orders) {
    try {
      ...
      if (order.table) {
        ...
        const table = await (Table as any).findById(order.table._id);
...
async function main() {
  try {
    ...
    if (earlyOrders.length === 0) {
      ...
      console.log("   Script дахин ажиллуулж confirm=true параметр өгнө үү:");
